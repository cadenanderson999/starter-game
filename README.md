...
A small, fully offline, single-player browser game: build a town by day, defend it by night.
...
**Day (75 s)** — place buildings, keep villagers happy.
...
| Key | Action |
| --- | --- |
| `1`–`8` | Pick a building (click the map to place, Shift-click to place several, drag to paint walls) |
...
- `style.css` — the look
- `game.js` — data tables (`DEFS`, `MOBS`, `TH_LEVELS`), simulation, canvas rendering, DOM UI and save/load

Tuning knobs live at the top of `game.js`: day/night length, building stats, mob stats and the
`waveFor()` function that decides what each night throws at you.
